...
setwd("C:/Users/nbreu/Desktop/Bachelor/Problem Set")
ps.name = "TheOpportunityCostofDebtAversion"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("ggplot2","readr","dplyr", "shiny", "knitr","stargazer","scales", "kableExtra", "sandwich","tidyr", "forcats") # character vector of all packages you load in the problem set
#name.rmd.chunks(sol.file)
create.ps(sol.file=sol.file, ps.name=ps.name, libs=libs, addons = "quiz",extra.code.file = "graphic_data.R")
...
The full paper by Martínez-Marquina and Shi (2024) can be accessed here: https://doi.org/10.1257/aer.20221509

This problem set replicates key findings from Martínez-Marquina and Shi (2024), using their publicly available experimental dataset from the openICPSR repository (AER-2022-1509) under a Creative Commons Non-Commercial license. All credit for data collection belongs to the original authors.

Various analyses, regressions, and visualizations are direct replications of the original study, following its methodology and data structure. While independently executed, they are designed to maintain consistency with the published results, ensuring accurate representation of the study’s findings.

For figures and regressions requiring additional data preparation, a dedicated preprocessing file (graphic_data.R) is included as an extra.code.file within the directory. This file is directly connected to the problem set, meaning that in multiple cases, users do not need to manually read in data—the required datasets are preprocessed and stored in a format that can be used immediately. The purpose of this setup is to offload complex data preparation and avoid unnecessary table creation, thereby enhancing clarity and making it easier for users to focus on interpreting results rather than handling raw data transformations.

**Structure of This Problem Set**

1. The Main Treatments

    – Understanding the experimental design and initial data analysis.

2. Behavior with Debt

    – Exploring how participants react to increased debt levels and whether they follow optimal financial strategies.

3. Additional Treatments

    – Examining variations of the experiment and how different conditions affect debt-related decision-making.

4. Debt Aversion

    – Literature review and theroretical model
...
References
    ...
    – Providing citations and further readings.

R Packages

    – Listing essential R packages used in this problem set.
...
💡 Tip: If you are unsure how to proceed with a task, look for hints or review the explanations provided in each exercise.

👉 Now, let’s get started and explore the fascinating effects of debt aversion on financial decision-making! 🚀
...
A key feature of the experiment was the dynamic allocation process, where participants strategically distributed an initial endowment across their accounts over multiple decision rounds. This setup mirrored real-world financial decision-making, where past allocations impact future investment opportunities. However, not all accounts remained accessible—some were restricted from direct allocations but continued accumulating interest, allowing for a causal comparison of financial behavior under different debt conditions.
...
By structuring the experiment this way, the study aimed to provide causal evidence on debt’s influence on financial priorities. Unlike observational studies, where pre-existing characteristics may confound results, this controlled setting ensured that any observed behavioral differences arose specifically from debt exposure rather than external economic conditions.
...
This study recruited 578 participants, who were randomly assigned to different treatment conditions. Each treatment group contained approximately 82 participants, with the actual number per group ranging from 77 to 86.
Participants were recruited via Amazon Mechanical Turk (MTurk), a widely used online platform for behavioral research. To ensure data quality and reliability, eligibility criteria included U.S. residency, completion of at least 50 Human Intelligence Tasks (HITs), and a minimum approval rating of 90%. Individuals who had previously participated in similar experiments were excluded to prevent familiarity effects from influencing decision-making.
As previously mentioned, the study spanned multiple decision rounds, and participants received automated reminders every six hours on decision-making days to minimize attrition. This method contributed to a high completion rate of 85%, with no significant differences in dropout rates between treatment groups.
Before making financial decisions, participants underwent a structured instructional phase designed to ensure a thorough understanding of the experiment. This phase included step-by-step demonstrations, followed by mandatory comprehension check questions that participants had to answer correctly before proceeding. Instructions were repeated before each decision round, ensuring consistent understanding. By the conclusion of the experiment, each participant had completed multiple rounds of instructional reinforcement, reducing the likelihood of errors caused by misinterpretation (Martínez-Marquina & Shi, 2024).
...
#read in csv-file and store
mainWork<-read_csv("main_work.csv")
```

#< quiz "dimension experimental data"

question: How many variables does our data contain?

sc:
    - 1290
    - 165*
    - There is another amount of variables.

success: You are right. We can see, that our data contains 1290 observations of 165 variables. That are quite a few to observe!

failure: Try again. Please look at the output again.
...
In this info task i will provide a description of important variables, we will need for the following exercise:

**day**: This variable provides information about the specific allocation day. It ranges from 0 to 4 since there where 4 allocation decisions at four different days in one week. Please note, that there are many missing values for day = 0. This is why rows for this value of day are not very interesting for us.

**participantid**: This variable provides information about the random id the subject got for the experiment

**treatment**: This variable provides information about the treatment the subject got randomly assigned to. It is a factor variable ranging from 0 (No Debt treatment) over 1 (Low Debt treatment) to 2 (High Debt treatment).

**initialendowment**: This variables describes the number of points the subject was able to allocate at the specific allocation day.

**points...**: The variables with the names starting with 'points...' and ending with the accounts names ( f.e. **pointssaving1**) are providing information about the amount of points the subject allocates to the specific account at the specific allocation day.
...
The dplyr package (Wickham et al., 2023) provides a powerful and intuitive way to manipulate data in R. A key feature of dplyr is the pipeline operator (%>%), which allows chaining multiple operations together in a clear and readable sequence. Instead of nesting functions, the pipeline operator passes the output of one function directly into the next, improving code clarity and efficiency.
...
**group_by()**: Groups data by one or more variables for aggregation.

**mutate()**: Creates new or modified variables.
...
```{r "1_b)_2", out.width="100%"}
#select relevant columns, filter for low-debt treatment (1)
main1<-mainWork%>%
    select(day, participantid, treatment, initialendowment, pointssaving1, pointssaving2, pointssaving3, pointssaving4, pointsdebt1, pointsdebt2)%>%
    filter(treatment==1)
#show results
...
#< hint
display("Use the `select()` function to extract only the relevant columns (day, participantid, treatment, initialendowment, etc.). Then, apply the `filter()` function to select only participants in the Low-Debt treatment (treatment == 1). Finally, display the first four rows using the `head()` function with `4` as the second argument.")
#>
```
...
**ggplot2** (Wickham, 2016) is a powerful tool for creating a wide variety of graphics in R. To start, you need to use the ggplot() function and specify the dataset as the first argument. Inside the aes() function, you define the mappings for the x and y axes. Additional specifications, such as the type of plot, scales, and faceting options, are added using the + operator.

For example, if you want to create a bar chart, you can use the geom_bar() function. Here’s how you would generate a simple bar plot using a dataset called testclass11, which contains test results  (variable **testresult**) for students in class 11:


**ggplot(testclass11, aes(x = name, y = testresult)) + 
  geom_bar(stat = "identity")**

In addition to that, many more specifications are possible, as you will see in the following exercises.
You might wonder about the role of stat = "identity". By default, geom_bar() counts the number of occurrences for each category on the x-axis. However, when using stat = "identity", R directly takes the values from the dataset instead of counting occurrences.
...
plot1<-ggplot(___, aes(x = share, y = mean_value, fill = share, label = round(mean_value, 2))) +
 geom_bar(stat = ___, width = 0.7) +
  ...
  labs(subtitle="Own illustration based on Martínez-Marquina & Shi, 2024, p. 1152, 'Figure 1'\n",
...
#>
  ...
  #sample solution
  library(ggplot2)
  plot1<-ggplot(plotfigure1, aes(x = share, y = mean_value, fill = share, label = round(mean_value, 2))) +
...
#< info "regression"
The following explanation of linear regression is based on Wooldridge (2020), which provides a foundational overview of regression analysis in econometrics.

Linear regression is a fundamental statistical method used to analyze the relationship between one or more independent variables (predictors) and a dependent variable (outcome). The primary goal is to estimate how changes in the independent variables affect the dependent variable.

**Basic Structure of a Linear Regression Model:**
A simple linear regression model takes the form: Y = ß0 + ß1X + e

where:
- Y is the dependent variable (outcome),
- X is the independent variable (predictor),
- ß0 is the intercept (the expected value of Y when x = 0),
- ß1 is the coefficient (showing how much Y changes with a one-unit decrease in X),
- e represents the error term (unexplained variation in Y)

In multiple linear regression, we extend this to multiple predictors which allows us to control for multiple factors simultaneously and assess their individual contributions to the outcome:

Y = ß0 + ß1x1 + ß2x2 + ... + ßnxn + e

**Interpreting Regression Output**:
After running a regression, the key elements to look for in the output are:

- Coefficients (ß): Indicate the estimated effect of each independent variable on Y.
- P-values: Show whether the effect of a variable is statistically significant (typically, p<0.05 is considered significant).
- R²: Measures how well the model explains the variability in the dependent variable (closer to 1 means a better fit).
- Standard errors: Indicate the uncertainty of the coefficient estimates.  

**Benefits of using linear regression**:

Linear regression is widely used in economics and social sciences because:

- It provides causal insights (when assumptions hold)
- It helps identify statistically significant relationships between variables
- It allows for prediction and policy evaluation.


**Using lm() in R**:
In R, linear regression models are estimated using the lm() function. The basic syntax is:

model <- lm(Y ~ X, data = dataset)
summary (model)

For multiple regression, simply add more predictors.

Throughout this problem set, you will use linear regression to test how different experimental treatments influence financial decision-making, helping to uncover systematic behavioral patterns in investment choices.


**Interpreting factor variables**

When a **factor variable like the treatment group** is included in a regression, the model automatically assigns one category as the baseline (reference group). The Intercept (ß0) represents the mean outcome for this baseline group, while the coefficient of the treatment variable (ß1) measures how much the outcome differs for the other group(s) compared to the baseline.

**Helpful additional packages**
For your initial understanding, please note the following:

We will use the stargazer package (Hlavac, 2022) for the regression output, as it provides a more aesthetically pleasing presentation. To do this, you only need to provide the model name, the display type (in our case, "text"), and the number of digits to be displayed in the stargazer() command.

Since the authors (Martínez-Marquina & Shi, 2024) conducted significant parts of their analysis in Stata and used robust standard errors, we will follow the same approach in R. These robust standard errors have the advantage of being heteroskedasticity-consistent, meaning they remain reliable even when the error variance is not constant in a regression model.
To implement this, we will use the vcovHC() function from the sandwich package (Zeileis, 2004; Zeileis, Köll, and Graham, 2020), setting the standard error type to "HC1" to ensure consistency with Stata's default robust standard errors. This requires an additional step compared to standard practice: we must compute these standard errors separately and then pass them to the stargazer() command.
#>
...
#< quiz "regtable"
question: Based on the regression results, which of the following statements is correct?
sc:
    - The coefficient labeled constant represents the initial allocation share for Low Debt, as we saw earlier.
    - By adding the constant coefficient and the treatment coefficient, we obtain the share of points allocated to Debt 1 by subjects in Low Debt treatment.*
    - Since the R² value is only 0.1279, our findings are statistically meaningless.
success: Correct! The sum of the constant and treatment coefficients (~0.317) confirms our earlier findings for Debt 1 in Low Debt. The coefficient for No Debt can also be verified from our regression output. 

failure: Not quite! Think about the role of the constant in a regression model and how the treatment coefficient modifies the allocation in the Low Debt group. Try again!
#>

After analyzing the regression output (Martínez-Marquina & Shi, 2024), we can now examine its implications in more detail. The results indicate a distinct difference in allocation behavior between the treatment groups. In the No Debt condition, the share of allocated points directed to Savings 3 was 12.9 percent, as captured by the constant term. In the Low Debt condition, this allocation pattern shifted, with 31.7 percent of the allocated points directed toward Debt 1 instead. This value is obtained by summing the constant and treatment coefficients (0.1294 + 0.1880).
The treatment coefficient of 0.1880 indicates that, on average, Low Debt participants allocated 18.8 percentage points more to Debt 1 than No Debt participants did to Savings 3. This shift is highly statistically significant (p < 0.01), confirming that the observed difference in allocation is unlikely to be due to random variation. The results suggest that the presence of debt is systematically associated with a reallocation of funds away from savings and toward debt repayment. While this pattern aligns with the behavioral tendencies observed in the earlier figure, it is important to note that the regressions do not establish a direct causal mechanism but rather provide statistical confirmation that allocation patterns differ systematically between the groups. The authors interpret these differences as evidence that debt influences financial decision-making by shifting participants' focus toward repayment, often leading them to deviate from payoff-maximizing behavior (Martínez-Marquina & Shi, 2024).
Although the R² value is relatively low (0.128), this is not a concern, as the goal of this analysis is to estimate the treatment effect rather than explain the full variance in allocation choices. 
...
Next, we will visualize the proportion of participants in each treatment group who maximized returns in all allocation decisions using a bar plot (Martínez-Marquina & Shi, 2024, p. 1153, "Figure 2"). 
...
hint("Ensure you use the pipe operator (%>%) to chain operations efficiently. When using summarise(), remember to name the new variable explicitly (mean_optimal = mean(...)).")
...
plot2 <- ggplot(data_fig2, aes(x = ifelse(treatment == 0, 0.2, 0.8), y = mean_optimal, , fill = factor(treatment))) +
  ...
  labs(
    ...
    x = "Return-maximizing in all decisions\n \nOwn illustration based on Martínez-Marquina, A., & Shi, M., 2024, p. 1153, 'Figure 2'"
  ) +
  theme_minimal() +
  theme(
...
#< quiz "return-maximizing"
question: Does the resulting plot match with our first findings from above?
sc:
  - Yes, it does. Here we can see that again many more subjects without debt maximize returns compared to subjects with debt.*
  - Partly.
  - No, it doesn't.
success: Correct! The percentage of return-maximizing subjects across all decisions is much higher for No Debt (38% vs. 13%). This indicates that subjects with debt tend to deviate more often from the return-maximizing strategy, reinforcing the earlier findings from the initial allocation analysis.

failure: Try again! Think about the difference between the groups and whether this aligns with our findings from the regression analysis and previous visualizations.
#>
...
These results are consistent with our previous findings on initial allocations, reinforcing the conclusion that participants in Low Debt systematically deviate from return-maximization—not only in their first decision but persistently across all decision rounds.
...
Although the R² value is relatively low (0.0859), this is expected, as the model is designed to isolate the treatment effect rather than maximize explanatory power. The strong statistical significance of the treatment coefficient reinforces the robustness of the findings.
...
**'demo_age_median'**: Provides information about the subjects age.

**'demo_sex'**: Provides information about the subjects gender.

**'demo_white'**: Provides information about the subjects race.

**'demo_collegeplus'**: Provides information about the subjects education.

**'demo_studentloan'**: Provides information about whether the participant has a student loan.

**'demo_holddebt'**: Provide information about whether the subject had any outstanding debt.

**'demo_covid'**: Provides information about the impact of covid on the subjects life.

**'error_count_total'**: Provides information about the number of errors in understanding questions across the four allocation days.
...
#< quiz "regression"
question: How does the inclusion of additional control variables affect the treatment effect in the second regression?

sc: 
  - The treatment effect disappears, indicating that differences in return-maximizing behavior between No Debt and Low Debt were entirely driven by other factors.
  - The treatment effect remains significant but slightly decreases.*
  - The treatment effect increases, meaning that controlling for other variables reveals an even stronger negative impact of debt on return-maximizing behavior.
  
success: Correct! The treatment effect remains highly significant but slightly decreases from -0.2558 to -0.2404, suggesting that while error tendencies and demographics play a role, debt still independently reduces return-maximizing behavior.

failure: Not quite! Look at the treatment coefficient before and after adding control variables. Does it disappear, decrease slightly, or become stronger?

#>

This second regression (Martínez-Marquina & Shi, 2024) expands the initial model by incorporating demographic characteristics and error tendencies to assess whether the treatment effect of Low Debt persists after controlling for additional individual factors.

The results confirm the hypothesis that debt significantly reduces the likelihood of consistently maximizing returns, even when accounting for individual differences. The treatment coefficient (-0.2404, p < 0.01) indicates that participants in Low Debt are 24.0 percentage points less likely to be return maximizers compared to those in No Debt. While this effect slightly decreases compared to the first regression (-0.2558 → -0.2404), it remains highly significant, suggesting that debt itself continues to bias financial decision-making, independent of other individual characteristics.
...
However, since the treatment effect remains strong, debt is not merely associated with poor financial decisions—it actively biases choices, leading to persistently lower financial returns.
...
Although the R² increases from 0.0859 to 0.2078, meaning that individual characteristics explain a greater share of variation in decision-making, the persistence of the treatment effect highlights the unique role of debt in shaping financial behavior. 
...
p3.1 <- ggplot(data_plotfig3, aes(x = tot_points_a3, y = percent, 
  ...
  labs(
    ...
    \n\nOwn illustration based on Martínez-Marquina, A., & Shi, M., 2024, p. 1154, 'Figure 3'",
...
A key observation is the sharp peak at exactly 900 points for Low Debt participants, indicating that a substantial share of subjects prioritized repaying their debt balance precisely to zero rather than allocating funds based on return-maximizing principles. Martínez-Marquina & Shi (2024) report that 33.7% of participants in Low Debt allocated exactly 900 points to Debt 1. We can't observe a similar pattern for the No-Debt group. The authors confirm that this pattern is statistically significant (p < 0.001), **indicating a systematic tendency to prioritize debt elimination over financial optimization**.
...
These differences suggest that debt creates a psychological constraint, leading individuals to adopt a rigid repayment approach even when financially suboptimal. The statistically significant gap (p < 0.001) between groups confirms that the presence of debt systematically alters financial decision-making, encouraging a strict focus on repayment rather than a flexible, return-maximizing strategy (Martínez-Marquina & Shi, 2024).
...
#< quiz "Debt 2"
question: What kind of difference in behavior can we observe compared to the plot before?

sc:
   - Fewer subjects in both treatments allocate points to this account.*
   - More subjects without debt completely repay their debt account.
   - The differences do not significantly affect overall behavior.
...
#< award "Master of the main treatments"

"Congratulations! You've uncovered parts of how debt shapes financial decisions!
No Debt participants allocated 73% of their initial endowment to the highest-return account, while Low Debt participants allocated only 47%, prioritizing debt repayment (32%) instead. This is associated with a 9.9% lower return for Low Debt participants. Over time, this pattern persisted: 38% of No Debt participants maximized returns across all decisions, compared to only 13% in Low Debt.

The allocation plots confirmed this bias: 33.7% of Low Debt participants allocated exactly 900 points to Debt 1, while 38.4% of No Debt participants allocated nothing to Savings 3—showing greater financial flexibility. For Debt 2 (5% interest), even fewer Low Debt participants allocated points, with 94% of those repaying it first clearing Debt 1, reinforcing a structured repayment order.

Regression analysis confirmed that Low Debt participants were significantly less likely to maximize returns, even when accounting for individual differences. This highlights how debt influences financial choices, leading parts of the participants to prioritize repayment over financial gains."
...
#read in data
mainWork<-read_csv("main_work.csv")
#filter for high-debt treatment (2) and show
main2<-mainWork%>%select(day, participantid, treatment, initialendowment, pointssaving1, pointssaving2, pointssaving3, pointssaving4, pointsdebt1, pointsdebt2)%>%filter(treatment==2)
head(main2, 4)

#< hint
display("Again, you will need to use the select() function to extract only the relevant variables (day, participantid, treatment, initialendowment, pointssaving1, pointssaving2, pointssaving3, pointssaving4, pointsdebt1, pointsdebt2) before filtering for the treatment group. In the next step, use the head() function to display the first four rows by setting the second argument to 4.")
#>
```
...
This aligns with the study’s hypothesis that debt repayment strategies depend on feasibility—when full repayment is impossible, some individuals continue reducing debt, while others prioritize alternative strategies
...
```{r "2_b)_1", results= 'asis', out.width="100%"}
#< task_notest
...
p4.1 <- ggplot(data_plot4.1, aes(x = tot_points_a3, y = percent, 
  ...
  labs(
    ...
    \n\nOwn illustration based on Martínez-Marquina, A., & Shi, M., 2024, p. 1155, 'Figure 4'",
    y = "Percentage of Subjects (log scale)"
  ) +
  theme_minimal() +
  geom_vline(xintercept = 900, linetype = "dashed", color = "black", linewidth = 0.8) +  
...
#< quiz "increasing debt - Debt 1"

question: Which statement is correct?

sc: 
    - Since fewer points are required to zero out Debt 1 in 'Low Debt', more subjects in this treatment fully repay their debt compared to 'High Debt'.*
    - Low Debt subjects seem to maximize returns more often because the share of participants allocating zero points to the account presented is higher compared to 'High Debt'.
    - Low Debt subjects maximize returns more often since they allocate fewer points to 'Debt 1' on average.
success: Correct!
...
#< quiz "increasing debt - Debt 2"

question: What can we derive from the plot?

sc:
    - More Low Debt subjects maximize returns considering allocations to Debt 2.
    - In both groups, there are more subjects allocating zero points to Debt 2 compared to Debt 1.*
    - In Low Debt, fewer subjects allocate zero points to Debt 2 compared to Debt 1.
success: Correct!
...
"Congratulations!
This exercise provided key insights into debt aversion and how individuals react when faced with high, unmanageable debt. The results illustrate that debt repayment behavior is not purely driven by financial optimization but is shaped by psychological biases.

Participants in the Low Debt treatment exhibited a strong debt account closure effect, with 34% fully repaying Debt 1, even when alternative strategies could have led to higher returns. However, in High Debt, where full repayment was unattainable for most, only 10% fully repaid Debt 1. Instead, behaviors diverged—some participants abandoned repayment entirely in favor of return maximization (27% allocated zero points to Debt 1), while others persisted in paying down debt, despite the financial cost.

The trend was even stronger for Debt 2 (5%), where 57% of High Debt participants allocated zero points, compared to 28% in Low Debt. This suggests that when repayment is infeasible, individuals prioritize higher-interest debt first or shift entirely toward wealth-maximizing strategies.

Regression results confirm that debt aversion intensifies under higher debt burdens, with High Debt participants allocating 9 percentage points more of their total points to debt repayment than Low Debt participants. The One-Shot Robustness Treatment further supports this by showing that participants in the No Debt condition were significantly more likely to optimize returns, while those in debt conditions allocated points toward repayment—even when it was not the best financial strategy. This highlights how debt aversion is not just about cost-benefit considerations but is driven by the psychological discomfort of carrying debt, leading individuals to prioritize debt reduction even at the expense of long-term financial gains (Martínez-Marquina & Shi, 2024)."
...
The Redistribution Treatment introduces an additional layer of financial flexibility, allowing participants to reallocate balances between Savings 1 and Savings 2. This experimental variation helps evaluate whether subjects strategically adjust their allocations to optimize returns or if they continue to exhibit debt-biased financial behavior (Martínez-Marquina & Shi, 2024).
...
#< quiz "redistribution treatment"
question: Which of the two subjects acted in a fully return-maximizing way?
sc:
    - Neither of them.*
    - The participant in Redistribution No-Debt.
    - The participant in Redistribution Debt.
    - Both.
success: Correct! Neither participant followed the fully return-maximizing strategy. While the participant in Redistribution No-Debt allocated all points to 'Savings 1', she missed the opportunity to **redistribute points from 'Savings 2' to 'Savings 1'**, which could have increased returns. The participant in Redistribution Debt actively redistributed points in every round, but **not optimally**, as she did not allocate all redistributable points to 'Savings 1', which would have maximized her returns.
...
In the Redistribution No-Debt treatment, the participant allocated all available points to Savings 1, which offers the highest return. However, redistribution options were not utilized, meaning the opportunity to move funds from Savings 2 to Savings 1—which would have increased returns—was overlooked.
...
By considering these factors, we see that neither participant strictly followed the fully return-maximizing strategy—one failed to optimize redistribution, while the other diversified allocations in a way that did not maximize long-term gains.
...
ggplot(df_cumul6, aes(x = cum_total_returns, y = prob, color = treatment_label)) +
  ...
  labs(
    x = "Total returns
    \n\nOwn illustration based on Martínez-Marquina, A., & Shi, M., 2024, p. 1158, 'Figure 6'",
...
#< quiz "redistribution returns"

question: Which key finding can we derive from this line plot?

sc:
    - Subjects in the Redistribution Debt group generate higher total returns than those in Redistribution No-Debt.
    - Subjects in Redistribution No-Debt attain the highest possible total return more often than participants in Redistribution Debt.*
    - Subjects in Redistribution No-Debt perform worse than those in Redistribution Debt.
...
#< quiz "borrowing treatment"
question: Which of the two subjects followed a completely return-maximizing strategy?
sc:
    - Neither of them.*
    - The participant in Borrow-Debt.
    - The participant in Borrow-Savings.
    - Both participants.
success: Correct! 
...
plotb <- ggplot(summary_combined, 
  ...
  labs(
    y = "Percent of Subjects",
    x = "\nOwn illustration based on Martínez-Marquina, A., & Shi, M., 2024, p. 1159, 'Figure 7'",
    fill = NULL
  ) +
  scale_x_continuous(
...
#< quiz "borrowing with debt"

question: What can we derive from the plot?

sc: 
    - When debt is involved, subjects borrow the maximum amount more often.
    - When Debt is not involved, subjects borrow the maximum amount more often.*
    - The plot does not provide any useful information about borrowing behavior.
...
The plot reveals a clear distinction between the two treatment groups. Subjects in the Borrow-Savings treatment are substantially more likely to borrow the maximum allowable amount than those in Borrow-Debt. The difference is substantial, with 62% of Borrow-Savings participants borrowing the full amount, compared to only 34% in Borrow-Debt. The authors claim that this effect is highly significant (p < 0.001) (Martínez-Marquina & Shi, 2024).
...
#< award "Master of the Experiment"

"Congratulations! This exercise demonstrated how debt influences financial behavior in Redistribution and Borrowing. 

The Redistribution Treatment showed that participants with debt struggled to maximize returns, often prioritizing early repayment over optimal investment. While redistribution allows for flexibility, many failed to use it effectively, leading to significantly lower returns compared to those without debt.
The Borrowing Treatment highlighted the strong psychological effect of debt framing. Participants were far less likely to borrow when it was labeled as "debt" even when doing so would have maximized their returns. This reluctance was particularly evident among those who otherwise optimized their allocations, confirming that debt aversion extends beyond repayment to borrowing decisions as well.

These findings suggest that debt aversion is not merely a rational response to financial constraints but a cognitive bias that distorts borrowing and repayment behavior. Beyond the experiment, this has broad implications for financial decision-making, from personal finance to credit markets. Understanding and addressing this bias could help individuals make more informed, financially sound choices (Martínez-Marquina & Shi, 2024)."
...
Martínez-Marquina and Shi (2024) introduce a theoretical model of debt aversion, incorporating a debt aversion parameter (λ) to quantify how individuals psychologically perceive and respond to debt. Their model demonstrates that individuals systematically overweight the burden of debt, perceiving each dollar of debt as $1.03–$1.07 in savings, leading them to prioritize debt repayment over investment, even when investment opportunities offer higher financial returns. The model further categorizes individuals into three distinct types based on their level of debt aversion: non-debt-averse individuals, who invest optimally; moderately debt-averse individuals, who prioritize repaying high-interest debt before investing; and highly debt-averse individuals, who avoid investing entirely until all debt is repaid, often leading to missed profitable opportunities.

The authors apply their theoretical model within the experimental setting, using it to explain the financial behavior of participants. The fact that the model’s predictions match the observed experimental behavior strengthens the causal link between debt aversion and financial decision-making. 
...
#< quiz "Debt Aversion 1"

question: What is debt aversion?
sc:
    - A preference for avoiding or quickly repaying debt, even when financially suboptimal.*
    - A tendency to maximize borrowing at any cost.
    - A rational strategy based purely on interest rates.
    - The exclusive use of savings for all financial decisions.

Success: You are right! Debt aversion is the tendency to avoid or quickly repay debt, even when it is not the most financially optimal choice.

failure: Try again!

#>

#< quiz "Debt Aversion 2"

question: According to behavioral research, why do individuals prioritize debt repayment over investment?

sc: 
    - Because repaying debt always provides a higher return than investing.
    - Due to psychological biases that influence financial decision-making.*
    - Because traditional economic theory suggests avoiding debt is always optimal.
    - Because banks encourage early repayment.
...
#< quiz "Debt Aversion 3"

question: What are key findings on Debt Aversion reported by Martínez-Marquina & Shi (2024)?
sc:
    - Many participants avoid investment if it involves taking on debt.
    - Framing borrowing as 'debt' significantly reduces participants' willingness to borrow, even when it would maximize financial returns.
    - Many participants prioritize debt repayment, even at the cost of financial gains.
    - All of the previous.*
...
Overall, Martínez-Marquina and Shi (2024) provide valuable insights into how debt aversion influences financial decision-making. By combining empirical analysis with experimental validation, the study offers a robust framework for understanding how individuals react to debt and how policy interventions could correct financially suboptimal behavior. Addressing these challenges through improved financial products, behavioral interventions, and targeted policy reforms could enable individuals to make better-informed financial choices, ultimately promoting economic mobility, business expansion, and long-term financial stability.

## Exercise References

- Amar, M., Ariely, D., Ayal, S., Cryder, C. E. and Rick, S. I. (2011) ‘Winning the battle but losing the war: The psychology of debt management’, Journal of Marketing Research, 48(SPL), pp. S38–S50.

- Besharat, A., Carrillat, F. A. and Ladik, D. M. (2014) ‘When motivation is against debt reduction: The impact of goal framing on consumer debt repayment decisions’, Journal of Public Policy & Marketing, 33(2), pp. 143–158.

- Callender, C. and Jackson, J. (2005) ‘Does the fear of debt deter students from higher education?’, Journal of Social Policy, 34(4), pp. 509-540. Available at: https://doi.org/10.1017/S004727940500913X.

- Gathergood, J., Mahoney, N., Stewart, N. and Weber, J. (2019) ‘How do individuals repay their debt? The balance-matching heuristic’, American Economic Review, 109(3), pp. 844-875. Available at: https://doi.org/10.1257/aer.20180288.

- Kettle, K. L., Trudel, R., Blanchard, S. J. and Häubl, G. (2016) ‘Repayment concentration and consumer motivation to get out of debt’, Journal of Consumer Research, 43(3), pp. 460–477.

- Martínez-Marquina, A. and Shi, M. (2024) ‘The Opportunity Cost of Debt Aversion’, American Economic Review, 114(4), pp. 1140–1172. Available at: https://doi.org/10.1257/aer.20221509.

- Nguyen, H. T., Freeman, J. and Gelb, D. (2020) ‘Entrepreneurial financing and debt aversion: Behavioral insights from small business owners’, Journal of Business Venturing Insights, 13, e00159. Available at:                                                            https://doi.org/10.1016/j.jbvi.2020.e00159.

- Prelec, D. and Loewenstein, G. (1998) ‘The Red and the Black: Mental Accounting of Savings and Debt’, Marketing Science, 17(1), pp. 4–28. Available at: https://doi.org/10.1287/mksc.17.1.4.

- Thaler, R. H. (1985) ‘Mental accounting and consumer choice’, Marketing Science, 4(3), pp. 199–214. Available at: https://doi.org/10.1287/mksc.4.3.199.
...
- Zeileis, A. (2004) ‘Econometric computing with HC and HAC covariance matrix estimators’, Journal of Statistical Software, 11(10), pp. 1–17. Available at: https://doi.org/10.18637/jss.v011.i10
